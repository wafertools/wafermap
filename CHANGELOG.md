...
- **`RenderOptions.showMetadataBadge` → `showIdentityHeader`, and `WaferMapController.setMetadataBadgeVisible` → `setIdentityHeaderVisible`.** The bottom-left canvas overlay became a real layout row above the map, so the old names described something that no longer exists. Hard renames with no alias: passing the old option is a type error, and at runtime it is ignored.
...
- **Three docs demos raised an `inferred-pitch` advisory on their own sample data.** `csv-data.html` and `comparison.html` passed `waferConfig.diameter` with no die pitch — and `csv-data.html`'s comment recommended exactly that ("supplying `waferConfig.diameter` is enough for `units: 'mm'`") — so both showed a geometry advisory on shipped demo data. Each now names the die pitch that actually places its grid on a 300 mm wafer: 10 × 10 mm for `dummy-fulldata`, 9.5 mm square for `large-test`, 5 mm square for the denser WM-811K grid. `geometry.html`'s card C is the deliberate diameter-only teaching case and keeps its advisory, now captioned as intentional. The data files and generators were never at fault: every fixture in `docs/data/` and `docs/examples/data/` builds clean with no geometry config.
