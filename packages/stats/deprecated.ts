...
/** @deprecated Removed in 0.31.0. `analyzeWaferMap` and `analyzeWaferLot` now return per-test pass rates as `stats.testSpecYield` (test limits), `stats.testFlagYield` (tester verdicts) and `stats.functionalYield`, with `stats.specVerdictDisagreementDies.` */
export const buildTestPassRateData = deprecated(buildTestPassRateDataImpl, 'buildTestPassRateData', `analyzeWaferMap and analyzeWaferLot now return per-test pass rates as stats.testSpecYield (test limits), stats.testFlagYield (tester verdicts) and stats.functionalYield, with stats.specVerdictDisagreementDies. ${DEPRECATION_ISSUES}`);
...
// ── Deprecated in 0.30.3, removed in 0.31.0 ─────────────────────────────────
...
export const renderFindingsReportHtml = deprecated(renderFindingsReportHtmlImpl, 'renderFindingsReportHtml', `Use renderWaferReportHtml(result, summary) or renderLotReportHtml(results): their Findings section is the same table, alongside the population and yield it was found in. ${DEPRECATION_ISSUES}`);
