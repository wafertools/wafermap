...
export function resolveTestNumber(activeTest: number, testDefs?: TestDef[]): { testNumber: number } {
  if (testDefs?.length) {
    const def = findTestDef(testDefs, activeTest) ?? testDefs[0];
    return { testNumber: def.testNumber };
  }
...
export interface ViewRect {
   ...
   * ranges. Absent only under a pass/fail display (`passFailDisplay: 'spec'`),
   * where the solid categorical fill is itself the indication and no marker is needed.
   */
  specMark?: 'failLow' | 'failHigh';
  /**
   * This die's bin is NOT in `passBins` — i.e. it failed. Set only in the bin
   * modes, where the bin IS the verdict.
   *
   * Note "failing" is not "bin 1" and not "any bin above 1": `passBins` is
   * caller-supplied and any bin can be a pass bin or a fail bin, including
   * bin 1. Anything deriving pass/fail from the bin NUMBER is wrong.
   *
   * The renderer draws nothing for this on its own — it is the input to
   * toCanvas's optional non-colour failure marking (`markFailingDies`), which
   * exists so pass/fail does not depend on hue alone.
   */
  binFail?: boolean;
}
...
export interface View {
  ...
  testDefs?: TestDef[];
  /** The test number being displayed in `value` plot mode — a `die.testValues` key, not a position. */
  activeTest: number;
  ...
  /**
   * Per-category die counts for `value` + `passFailDisplay: 'spec'` — drives the spec legend. Counts only
...
export interface ViewOptions {
   ...
   * the renderers set this from `WaferMapResult.passBins`, so a map agrees with
   * its own yield figure; only a hand-rolled pipeline calling `buildView`
  ...
  testDefs?: TestDef[];
  /**
   * The test number to display in `value` plot mode — a `die.testValues` key
   * (`TestDef.testNumber`), not a position. Default `0`.
 ...
 * test's number so the active test can be located. Unifies the testValues path and
 * the recorded pass/fail path (`testPass`) so the tooltip builder has a single source. Skips tests with no
 * result for this die.
 *
...
/** Options for {@link buildHoverText}. All fields are optional. */
export interface HoverTextOptions {
  testDefs?: TestDef[];
  hbinDefs?: BinDef[];
  sbinDefs?: BinDef[];
  fallbackFormat?: 'si' | 'engineering';
  /** Lot-stack aggregation method name — shown so the reader knows what the scalar means. */
  aggrMethod?: string;
...
export function buildHoverText(
  ...
  if (plotMode === 'stackedValues') {
    // The aggregated scalar is stored at testValues[0].
    ...
    if (plotMode === 'value' && testRows.length) {
      ...
      const activeVal = getDieTestValue(die, testNumber);
...
export function generateTextOverlay(
  ...
  return dies.flatMap((die, i) => {
    ...
    if (plotMode === 'value' && passFailDisplay === 'test') {
      ...
      const v = getDieTestValue(die, tn);
      if (v === undefined) return [];
...
function pushDieRectangles(
  ...
  /** Which bins count as passing — see ViewRect.binFail. Any bin can be either. */
  passBinSet: Set<number>,
  ...
  if (plotMode === 'value') {
    ...
    const value = getDieTestValue(die, testNumber);
  ...
  const aggValue = getDieTestValue(die, 0);
...
export function buildView(
  ...
  let { testNumber: activeTestNumber } = resolveTestNumber(activeTest, testDefs);
  ...
  if (explicitRange) {
    ...
    for (const die of dies) {
      const v = isStacked
        ? getDieTestValue(die, 0)
        : getDieTestValue(die, activeTestNumber);
      if (v !== undefined) {
        if (v < lo) lo = v;
        if (v > hi) hi = v;
  ...
  // Visual kerf between dies: 1 mm, never more than 12% of the smallest die.
  const DIE_GAP_MM = 1;
  const gap = minDim !== Infinity ? Math.min(DIE_GAP_MM, minDim * 0.12) : DIE_GAP_MM;
  ...
  for (let i = 0; i < dies.length; i++) {
    ...
    if (specCounts && !die.partial && !die.edgeExcluded) {
      ...
      const cat = classifySpec(getDieTestValue(die, activeTestNumber), activeTestDef);
